    ...
    processGraph(allInputs, graphInputs) {
        ...
        if (allInputs.needSchemaProcessing || !this._options.ignoreJsonRefs) {
            ...
            do {
                ...
                if (!unionsDone) {
                    this.time("flatten unions", () => ([graph, unionsDone] = FlattenUnions_1.flattenUnions(graph, stringTypeMapping, conflateNumbers, shouldFlattenUnions, true, debugPrintReconstitution)));
                }
        ...
        do {
            this.time("flatten unions", () => ([graph, unionsDone] = FlattenUnions_1.flattenUnions(graph, stringTypeMapping, conflateNumbers, shouldFlattenUnions, false, debugPrintReconstitution)));
        } while (!unionsDone);
