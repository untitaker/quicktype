...
function sourceFromFileOrUrlArray(name, filesOrUrls, httpHeaders) {
    return __awaiter(this, void 0, void 0, function* () {
        const samples = yield Promise.all(filesOrUrls.map(file => NodeIO_1.readableFromFileOrURL(file, httpHeaders)));
        ...
        function readFilesOrURLsInDirectory(d) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                for (let file of files) {
                    ...
                    if (file.endsWith(".url") || file.endsWith(".json")) {
                        sourcesInDir.push({
                            kind: "json",
                            name,
                            samples: [yield NodeIO_1.readableFromFileOrURL(fileOrUrl, httpHeaders)]
                        });
                    }
                        ...
                        graphQLSchema = yield NodeIO_1.readableFromFileOrURL(fileOrUrl, httpHeaders);
                        ...
                        graphQLSources.push({
                            ...
                            name,
                            schema: undefined,
                            query: yield get_stream_1.getStream(yield NodeIO_1.readableFromFileOrURL(fileOrUrl, httpHeaders))
                        });
                    }
                }
                if (graphQLSources.length > 0) {
                    ...
                    const schema = quicktype_core_1.parseJSON(yield get_stream_1.getStream(graphQLSchema), "GraphQL schema", graphQLSchemaFileName);
...
function getSourceURIs(options) {
    return __awaiter(this, void 0, void 0, function* () {
        if (options.srcUrls !== undefined) {
            const json = quicktype_core_1.parseJSON(yield NodeIO_1.readFromFileOrURL(options.srcUrls, options.httpHeader), "URL grammar", options.srcUrls);
        ...
        for (const source of sources) {
            switch (source.kind) {
                ...
                case "json":
                    ...
                    break;
                case "schema":
                    yield inputData.addSource("schema", source, () => new quicktype_core_1.JSONSchemaInput(new NodeIO_1.FetchingJSONSchemaStore(httpHeaders), [], additionalSchemaAddresses));
                    break;
                default:
                    return quicktype_core_1.assertNever(source);
            }
        ...
        switch (options.srcLang) {
            case "graphql":
                ...
                for (const queryFile of options.src) {
                    ...
                    const query = yield get_stream_1.getStream(yield NodeIO_1.readableFromFileOrURL(queryFile, options.httpHeader));
